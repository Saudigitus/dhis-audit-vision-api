...
def save_notification(payload: NotificationConfigCreate, db: Session = Depends(get_db)):
    return notification_crud.create(db, payload_in=payload)


@router.get("")
def list_notifications(request: Request,  page: int = 1, pageSize: int = 20,  db: Session = Depends(get_db)):
...
def remove_config(request: Request, id: str, db: Session = Depends(get_db)):
